...
export default function SkillsSection() {
  ...
  return (
    <section className="pt-28 pb-20 container mx-auto">
      <div className="mb-8">
        <h2 className="md:text-5xl text-3xl underline font-bold text-center text-gray-800 mt-1">Tech Stack</h2>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
        <AnimatePresence>
          {filteredSkills.map((skill, index) => (
            <motion.div
              key={skill.name}
              className="flex flex-col items-center z-10 justify-center p-6 border border-gray-200 rounded-md bg-white hover:shadow-md transition-shadow"
            ...
            >
              {skill.icon}
              <span className="mt-3 text-center text-gray-700">{skill.name}</span>
            </motion.div>
          ))}
        </AnimatePresence>
      </div>
